...
class LocalCorpusKnowledgeBase:
    def __init__(self, corpus_dir: Path | str, *, min_score: float = _MIN_SCORE) -> None:
        ...
        self._cache: tuple[Chunk, ...] | None = None
    ...
    async def retrieve(self, queries: Sequence[str], *, top_k: int = 6) -> RetrievalOutcome:
        ...
        for query in queries:
            ...
            for chunk in documentos:
                score = self._score(objetivo, chunk)
    ...
    # -- interno --------------------------------------------------------
    def _score(self, objetivo: set[str], chunk: Chunk) -> float:
        cuerpo = _tokens(chunk.text) | _tokens(chunk.document_id.replace("-", " ").replace("_", " "))
        if not cuerpo:
            return 0.0
        comunes = objetivo & cuerpo
        return len(comunes) / len(objetivo)

    def _documents(self) -> tuple[Chunk, ...]:
            ...
            for ruta in sorted(self._dir.rglob("*")):
                ...
                chunks.append(
                    Chunk(
                        ...
                        text=ruta.read_text(encoding="utf-8").strip(),
                        score=0.0,
                        metadata=self._metadata(ruta),
                    )
                )
        self._cache = tuple(chunks)
        return self._cache
