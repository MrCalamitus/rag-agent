...
def profile_not_found(slug: str, disponibles: tuple[str, ...] = ()) -> AgentError:
    """El cliente pidió un tema que este despliegue no sirve.

    Se enumeran los disponibles: con un solo servicio atendiendo varios temas,
    un slug mal escrito es el error más probable y el más barato de corregir si
    la respuesta dice cuáles hay."""
    conocidos = f" Disponibles: {', '.join(disponibles)}." if disponibles else ""
    return AgentError(
        message=f"El perfil solicitado '{slug}' no existe.{conocidos}",
        type=ErrorType.INVALID_REQUEST,
        param="profile",
